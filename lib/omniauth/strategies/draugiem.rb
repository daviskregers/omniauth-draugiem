  ...
  module Strategies
    ...
    class Draugiem
      ...
      def request_phase
        ...
        redirect "//api.draugiem.lv/authorize/?#{query_string}"
      end

      def callback_phase
        if request.params['dr_auth_status'] == 'ok' && request.params['dr_auth_code']
          response = RestClient.get('//api.draugiem.lv/json/', { :params => draugiem_authorize_params(request.params['dr_auth_code']) })
